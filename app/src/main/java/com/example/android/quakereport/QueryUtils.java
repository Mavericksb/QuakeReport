...
public final class QueryUtils {
    ...
    private static String earthQuakeUrl = "";
    /**
    ...
    private static String makeHttpRequest(URL url) throws  IOException{
        ...
        try{
            ...
            if(urlConnection.getResponseCode() == 200){
                inputStrem = urlConnection.getInputStream();
                jsonResponse = readFromStream(inputStrem);
            }
            else
            {
                Log.e(LOG_TAG, "Error reading from stream, response code " + urlConnection.getResponseCode());
            }
        }
        catch(IOException e){
            Log.e(LOG_TAG, e.getMessage());
    ...
    private static List<Earthquake> extractFromJson(String earthquakeJSON) {
        ...
        try {
            ...
            JSONObject query = new JSONObject(earthquakeJSON);
            JSONArray features = query.getJSONArray("features");
            for(int i = 0; i < features.length(); i++ )
            {
                ...
                earthQuakeUrl = properties.getString("url");
                earthquakes.add(new Earthquake(magnitude, location, date, earthQuakeUrl));
